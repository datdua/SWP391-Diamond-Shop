...
export const createCertificate = async (certificate) => {
  try {
    ...
    const response = await axios.post(
      "http://localhost:3000/api/certificates/manager/create",
      certificate,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
...
export const updateCertificate = async (certificateId, certificate) => {
  try {
    ...
    const response = await axios.put(
      `http://localhost:3000/api/certificates/manager/update/${certificateId}`,
      certificate,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
...
export async function deleteCertificate(certificateIDs) {
  try {
    const token = localStorage.getItem("jwt");
    const response = await axios.delete("http://localhost:3000/api/certificates/manager/delete", 
      { 
        headers: { Authorization: `Bearer ${token}` },
        data: certificateIDs
       }
...
export const getCertificateByPage = async (page = 1, size = 9) => {
  try {
    const response = await axios.get(
      `http://localhost:3000/api/certificates/paged?page=${page}&size=${size}`
    );
    return response.data;
  } catch (error) {
