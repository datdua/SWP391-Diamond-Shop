...
export const addDiamondToCart = async (accountID, diamondId, quantity) => {
  try {
    const token = localStorage.getItem('jwt');
    const response = await axios.post(
      `https://www.thediamondstore.site/api/cart/customer/add?accountID=${accountID}&diamondID=${diamondId}&quantity=${quantity}`,
      {},
      {
        headers: {
          Authorization: `Bearer ${token}`
...
export const getAllCartItems = async (accountID) => {
  try {
    ...
    const response = await axios.get(
      `https://www.thediamondstore.site/api/cart/customer?accountID=${accountID}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
...
export const removeCartItem = async (cartID) => {
  ...
  try {
    const response = await axios.delete(
      `https://www.thediamondstore.site/api/cart/customer/remove/${cartID}`,{
        headers: {
          Authorization: `Bearer ${token}`
          }
      }
...
export const getTotalCart = async (accountID) => {
  try {
    const token = localStorage.getItem("jwt");
    const response = await axios.get(
      `https://www.thediamondstore.site/api/cart/customer/totalCart?accountID=${accountID}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
export const updateCart = async (cartId, accountId, diamondId, jewelryId, quantity, sizeJewelry) => {
  const token = localStorage.getItem('jwt')
  try {
      let url = `https://www.thediamondstore.site/api/cart/customer/update/${cartId}?accountID=${accountId}&quantity=${quantity}`;
