...
export async function updateAccount(accountID, accountDetails) {
  ...
  try {
    const response = await axios.put(
      `https://www.thediamondstore.site/api/api/customers/customer/update/${accountID}`,
      accountDetails,
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
export const getContactInfo = async (accountId) => {
  const token = localStorage.getItem('jwt')
  try {
    const response = await axios.get(`https://www.thediamondstore.site/api/accounts/customer/customer/contactInfo/${accountId}`,{
      headers: {
        Authorization: `Bearer ${token}`
      }
    });
